...
const SENSOR_STABILITY = Math.max(
  0,
  Math.min(0.98, AR_CONSTANTS.SENSOR.STABILITY),
);
const SENSOR_RESPONSE = 1 - SENSOR_STABILITY;
const HEADING_SMOOTHING_ALPHA = Math.max(0.02, SENSOR_RESPONSE);
const ORIENTATION_SMOOTHING_ALPHA = Math.max(
  0.02,
  SENSOR_RESPONSE * 0.6,
);
const HEADING_DEADBAND_DEGREES = SENSOR_STABILITY;
