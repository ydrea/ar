 ...
 private async startDeviceMotion() {
  ...
  this.deviceMotionSub = DeviceMotion.addListener((motion) => {
    ...
    if (rot.qx !== undefined) {
      ...
      // Log actual values to see if they change
      // Tlog(`🧭 Raw: yaw=${yaw.toFixed(3)}, pitch=${pitch.toFixed(3)}, roll=${roll.toFixed(3)}`);
      // Tlog(`🧭 Quat: w=${orientation.w.toFixed(3)}, x=${orientation.x.toFixed(3)}, y=${orientation.y.toFixed(3)}, z=${orientation.z.toFixed(3)}`);
    } else {
      return;
    }
...
function geoToENU(
  ...
  const north = -Math.sin(φ1) * Math.cos(λ1) * dx - Math.sin(φ1) * Math.sin(λ1) * dy + Math.cos(φ1) * dz;
  const up = Math.cos(φ1) * Math.cos(λ1) * dx + Math.cos(φ1) * Math.sin(λ1) * dy + Math.sin(φ1) * dz;
  
  // Debug: should be small numbers (meters)
  // Rlog(`📍 ${lat2},${lon2}: ENU east=${east.toFixed(1)}m, north=${north.toFixed(1)}m, up=${up.toFixed(1)}m`);
...
function projectToScreen(
  ...
  // Debug: log one POI each frame
  if (Math.random() < 0.01) {
    Tlog(`📐 depth=${depth.toFixed(1)}m, corr=(${correctedX.toFixed(0)}, ${correctedY.toFixed(0)}), screen=(${screenX.toFixed(0)}, ${screenY.toFixed(0)}), visible=${visible}`);
  }
