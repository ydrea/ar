...
export class ARGestureController {
  ...
  createGesture() {
    ...
    return Gesture.Pan()
      ...
      .onTouchesDown((event) => {
        // This is the correct event name for when touches start
      ...
      .onTouchesMove((event) => {
        // This is the correct event name for touch updates
        if (event.allTouches.length !== 2) return;
      ...
      .onTouchesUp((event) => {
        // This is the correct event name for when touches end
        if (this.gestureMode === "horizontal") {
          let finalFOV = this.fov;
          if (this.isRubberBanding) {
            finalFOV = Math.max(
              AR_CONSTANTS.FOV.MIN,
              Math.min(AR_CONSTANTS.FOV.MAX, this.fov),
            );
            if (this.callbacks) {
              runOnJS(this.callbacks.onFOVChange.bind(this.callbacks))(
                finalFOV,
          ...
          if (this.isRubberBanding) {
            finalMin = Math.max(
              AR_CONSTANTS.DISTANCE.MIN,
              Math.min(this.maxDistance - 1000, this.minDistance),
            );
            finalMax = Math.min(
              AR_CONSTANTS.DISTANCE.MAX,
              Math.max(this.minDistance + 1000, this.maxDistance),
            );
  ...
  private getExcessForLimit(limit: LimitType): number {
    switch (limit) {
      ...
      case "zoom":
        return Math.max(
          0,
          Math.abs(this.cameraZoom - (this.cameraZoom > 1 ? 1 : 0)),
        );
      case "fov":
        if (this.fov < AR_CONSTANTS.FOV.MIN)
          return AR_CONSTANTS.FOV.MIN - this.fov;
        if (this.fov > AR_CONSTANTS.FOV.MAX)
          return this.fov - AR_CONSTANTS.FOV.MAX;
        return 0;
      default:
        return 0;
    }
