...
export default function ARBetaLandscapeView() {
      ...
      <View style={styles.container} onLayout={handleLayout}>
        ...
        <Svg style={StyleSheet.absoluteFill} pointerEvents="none">
          ...
          {projectedPOIs.map((poi) => {
            ...
            if (!poi.isOffscreen && !isTooFar && !isTooClose) return null;
            ...
            if (isTooFar) {
              triX = centerX + Math.cos(angle) * (viewport.width / 2 - 40);
              triY = 50;
              triAngle = -Math.PI / 2;
              strokeColor = `rgba(255, 200, 100, ${Math.max(0.4, opacity)})`;
            } else if (isTooClose) {
              triX = centerX + Math.cos(angle) * (viewport.width / 2 - 40);
              triY = viewport.height - 50;
              triAngle = Math.PI / 2;
              strokeColor = `rgba(100, 200, 255, ${Math.max(0.4, opacity)})`;
            } else {
              const edgeDistance = 40;
              triX = centerX + Math.cos(angle) * (viewport.width / 2 - edgeDistance);
              triY = centerY + Math.sin(angle) * (viewport.height / 2 - edgeDistance);
              triAngle = angle;
            }
