...
//   },
// });
// ui/ActiveLimitIndicator.tsx - Minimal version
...
import { GestureMode, LimitType } from "@/cumquat/gestures/ArGestureControler";
...
export const ActiveLimitIndicator: React.FC<Props> = ({
  activeLimit,
  intensity,
  gestureMode,
}) => {
  const animatedStyle = useAnimatedStyle(() => {
    const opacity =
      intensity > 0.1 ? withTiming(Math.min(1, intensity * 2)) : withTiming(0);
    const scale = withSpring(1 + intensity * 0.3);
  ...
  if (!activeLimit || intensity < 0.1) return null;
  ...
  return (
    <Animated.View style={[styles.container, animatedStyle]}>
      <View style={styles.indicator}>
        <Text style={styles.label}>{getLimitLabel()}</Text>
        <View style={[styles.bar, { width: Math.min(100, intensity * 200) }]} />
      </View>
    </Animated.View>
  );
};
