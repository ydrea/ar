// cumquat/gestures/IntuitiveGestureController.ts
...
interface IntuitiveGestureCallbacks {
...
export class IntuitiveGestureController {
  private callbacks: IntuitiveGestureCallbacks | null = null;
  ...
  // Which finger is moving (for vertical gestures)
  ...
  // Sensitivity
  private readonly VERTICAL_PIXEL_TO_METER = 400; // meters per 100 pixels
  private readonly VERTICAL_PIXEL_TO_ZOOM = 0.003;
  private readonly HORIZONTAL_PIXEL_TO_FOV = 0.2; // degrees per pixel
  private readonly RUBBER_BAND_FACTOR = 0.3;
  
  setCallbacks(callbacks: IntuitiveGestureCallbacks) {
    ...
    return Gesture.Pan()
      ...
      .onTouchesStart((event) => {
        ...
        if (deltaX > deltaY * 1.5) {
          ...
          Tlog(`🎯 Horizontal pinch - FOV control: ${this.fov.toFixed(1)}°`);
          
        } else {
          // Vertical pinch - Distance control
          // Track which finger is top vs bottom
          ...
          Tlog(`🎯 Vertical pinch started - Min: ${(this.minDistance/1000).toFixed(1)}km, Max: ${(this.maxDistance/1000).toFixed(1)}km`);
        }
      ...
      .onTouchesMove((event) => {
        ...
        if (this.gestureMode === 'horizontal') {
          // Horizontal FOV control (unchanged, works intuitively)
          ...
          // Pinching in (fingers closer together) = decrease FOV (zoom in)
          ...
          // Vertical gesture - determine which finger is moving
          ...
          // Detect which finger(s) are moving significantly
          ...
          // Apply intuitive mappings
          if (this.gestureMode === 'adjustMax') {
            // TOP finger moves UP = increase MAX distance (see further)
            // TOP finger moves DOWN = decrease MAX distance (see less far)
            const maxDelta = -topDeltaY; // Negative because moving UP is negative Y
            ...
            // Also slightly adjust camera zoom (zoom in when increasing max distance)
            ...
            // BOTTOM finger moves UP = increase MIN distance (near clip further away)
            // BOTTOM finger moves DOWN = decrease MIN distance (near clip closer)
            const minDelta = -bottomDeltaY; // Negative because moving UP is negative Y
            ...
            // Also slightly adjust both distances for context
      ...
      .onEnd(() => {
        if (this.gestureMode === 'horizontal') {
          ...
          // Snap back vertical values if rubber banding
          ...
          Tlog(`✅ Vertical pinch ended - Mode: ${this.gestureMode}, Min: ${(finalMin/1000).toFixed(1)}km, Max: ${(finalMax/1000).toFixed(1)}km, Zoom: ${finalZoom.toFixed(3)}`);
